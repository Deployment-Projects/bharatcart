...
export const CartProvider = ({ children }) => {
  ...
  const sortItemsStable = (items) =>
    [...items].sort((a, b) => {
      ...
      const aProductId = Number(a?.productId ?? a?.product?.id ?? Number.MAX_SAFE_INTEGER);
      const bProductId = Number(b?.productId ?? b?.product?.id ?? Number.MAX_SAFE_INTEGER);
  ...
  const removeFromCart = async (item) => {
    ...
    if (!effectiveToken) {
      ...
      const updated =
        cartItems.filter(
          i => (i.productId ?? i.product?.id) !== (item.productId ?? item.product?.id)
        );
  ...
  const updateQuantity = async (item, qty) => {
    ...
    if (!item.id) {
      const productId = item.productId ?? item.product?.id;
      if (!productId) {
        console.log("Cannot update quantity: missing cart item id and product id");
        return;
      }
